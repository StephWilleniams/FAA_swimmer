...
void update_pos_RKII(int na, double xa[][4], double rA, double rSeg[], double sDV[], int np, double xp[][3], double rP, double vS, double sigR, double sigT, double xL, double xR, double yB, double yT, double f0, double fR, double fricPar, double fricPerp, double kickFreq, double kickStr, double dT, double DT, mt19937& gen){
    ...
    double xtemp_p[np][3]; // Temporary position store.
    ...
    for (int n=0; n<na; n++){ // Loop over particle index.
        ...
        F1a[n][2] = F1a[n][3] + (1/(fR*f0))*Fa[n][2] - kickStr*cos(xa[n][2])/abs(cos(xa[n][2]))*(xa[n][1])/abs(xa[n][1])*xa[n][3];
    ...
    for (int n=0; n<na; n++){ // Loop over particle index.
        ...
        F2a[n][2] = (1/(fR*f0))*Fa[n][2] - kickStr*cos(xa[n][2])/abs(cos(xa[n][2]))*(xa[n][1])/abs(xa[n][1])*xa[n][3];
    }
    ...
    for (int n=0; n<na; n++){ // Loop over particle index.
        ...
        if( (xa[n][1] > yT - 1.5*rA) || (xa[n][1] < yB + 1.5*rA) ){
            xa[n][3] = max(xa[n][3],ceil(urand(0,1,gen)-(exp(-kickFreq)))); // Determine if a kick is happening or if one is initiated.
        } else {
            xa[n][3] = 0;
        }
    ...
    for (int n=0; n<np; n++){ // Loop over particle index.
        ...
        xp[n][2] = ceil(abs(F1p[n][0] + F2p[n][0]));
